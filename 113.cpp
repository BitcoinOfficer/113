...
// ============================================================================
// SECTION 45: HISTORICAL ISSUE DATABASE
// ============================================================================
// Suppresses rediscovery of known CVEs and documented vulnerability classes.
// Every candidate is checked against this database before emission.
...
public:
    struct KnownIssue {
        ...
        std::string fixed_in;
        std::vector<std::string> code_fingerprints;
    };
    ...
    bool matches_known_issue(const Finding& finding) const {
        for (const auto& ki : known_issues_) {
            if (matches_fingerprint(finding, ki)) return true;
        }
        return false;
    }

    std::string get_matching_cve(const Finding& finding) const {
        for (const auto& ki : known_issues_) {
            if (matches_fingerprint(finding, ki)) return ki.cve_id;
        }
        return "";
    }

    bool is_known_class(const std::string& evidence) const {
        for (const auto& kc : known_classes_) {
            if (evidence.find(kc) != std::string::npos) return true;
        }
        return false;
    }

    bool is_fixed_in_version(const std::string& cve, const std::string& version) const {
        for (const auto& ki : known_issues_) {
            if (ki.cve_id == cve) {
                if (version.find("24.") != std::string::npos && ki.fixed_in.find("0.") == 0) return true;
                return version >= ki.fixed_in;
            }
        }
        return false;
    }
...
private:
    HistoricalIssueDatabase() {
        known_issues_ = {
            {"CVE-2018-17144", "Duplicate input inflation", "duplicate.*input.*CheckTransaction",
             "0.14.0-0.16.2", "0.14.3", {"CheckTransaction", "vInOutPoints", "duplicate inputs"}},
            {"CVE-2021-31876", "BIP125 RBF inheritance defect", "BIP125.*replacement.*inherit",
             "0.12.0-0.21.0", "22.0", {"BIP125", "SignalsOptInRBF", "replacement"}},
            {"CVE-2024-35202", "Compact block crash", "compact.*block.*crash",
             "pre-25.0", "25.0", {"cmpctblock", "BlockTransactionsRequest"}},
            {"CVE-2012-2459", "Merkle tree duplicate tx", "merkle.*duplicate.*mutation",
             "pre-0.6.1", "0.6.1", {"ComputeMerkleRoot", "mutated"}},
            {"CVE-2010-5139", "Value overflow 184B BTC", "value.*overflow.*int64",
             "pre-0.3.10", "0.3.10", {"MAX_MONEY", "MoneyRange", "nValueOut"}},
            {"KNOWN-BDB-ERASE", "BDB deleted record persistence", "BDB.*Erase.*page.*zero",
             "all-BDB-versions", "n/a", {"Erase", "walletdb", "deleted"}},
            {"KNOWN-PBKDF2-WEAK", "PBKDF2-SHA1 iteration weakness", "PBKDF2.*iteration.*brute",
             "all-versions", "n/a", {"PKCS5_PBKDF2", "nDeriveIterations"}},
            {"KNOWN-AES-CBC-NO-MAC", "AES-CBC without authentication", "AES.*CBC.*no.*MAC.*HMAC",
             "all-versions", "n/a", {"EVP_aes_256_cbc", "HMAC", "GCM"}},
            {"KNOWN-MEMCLEANSE", "Generic memory cleanse observation", "memory_cleanse.*not.*called",
             "all-versions", "n/a", {"memory_cleanse", "OPENSSL_cleanse"}},
            {"KNOWN-SIGHASH-SINGLE", "SIGHASH_SINGLE hash=1 bug", "SIGHASH_SINGLE.*hash.*1",
             "all-versions", "n/a", {"SIGHASH_SINGLE", "SignatureHash"}},
        };
        known_classes_ = {
            "memory_cleanse not called", "OPENSSL_cleanse not called",
            "BDB Erase", "deleted page still", "plaintext private key bytes",
            "PBKDF2", "nDeriveIterations", "EVP_BytesToKey",
            "AES-256-CBC WITHOUT", "no HMAC", "no GCM",
            "CVE-2018-17144", "duplicate input",
            "SIGHASH_SINGLE", "hash=1",
            "not wiped before function exit",
            "not explicitly wiped",
        };
    }

    bool matches_fingerprint(const Finding& f, const KnownIssue& ki) const {
        int match_score = 0;
        for (const auto& fp : ki.code_fingerprints) {
            if (f.evidence.find(fp) != std::string::npos) match_score++;
            if (f.function_name.find(fp) != std::string::npos) match_score++;
        }
        if (!ki.pattern.empty()) {
            try {
                std::regex re(ki.pattern, std::regex::icase);
                if (std::regex_search(f.evidence, re)) match_score += 2;
            } catch (...) {}
        }
        return match_score >= 2;
    }

    std::vector<KnownIssue> known_issues_;
    std::vector<std::string> known_classes_;
};

// ============================================================================
// SECTION 46: STRUCTURAL FINGERPRINT ENGINE
// ============================================================================
// Generates structural fingerprints for code patterns to detect whether
// a finding represents a genuinely novel code structure vs a known pattern.

class StructuralFingerprintEngine {
public:
    struct CodeFingerprint {
        ...
        bool has_crypto_ops;
        bool has_secret_access;
    };

    CodeFingerprint fingerprint_function(const std::shared_ptr<ASTNode>& func_node,
                                          const std::string& raw_content) {
        CodeFingerprint fp;
        if (!func_node) return fp;
        fp.function_signature = func_node->type_name + " " + func_node->name;
        fp.branch_count = 0;
        fp.loop_count = 0;
        fp.has_error_handling = false;
        fp.has_crypto_ops = false;
        fp.has_secret_access = false;

        std::function<void(const std::shared_ptr<ASTNode>&)> walk =
            [&](const std::shared_ptr<ASTNode>& node) {
            if (node->type == ASTNodeType::IfStmt || node->type == ASTNodeType::SwitchStmt)
                fp.branch_count++;
            if (node->type == ASTNodeType::ForStmt || node->type == ASTNodeType::WhileStmt ||
                node->type == ASTNodeType::DoStmt)
                fp.loop_count++;
            if (node->type == ASTNodeType::TryStmt || node->type == ASTNodeType::CatchStmt)
                fp.has_error_handling = true;
            if (node->type == ASTNodeType::CallExpr) {
                fp.called_functions.push_back(node->name);
                auto& kb = SecretTypeKnowledgeBase::instance();
                if (kb.is_crypto_function(node->name)) fp.has_crypto_ops = true;
                if (kb.is_secret_function(node->name)) fp.has_secret_access = true;
            }
            for (const auto& tok : node->tokens) {
                if (tok.type == TokenType::Identifier) {
                    auto& kb = SecretTypeKnowledgeBase::instance();
                    if (kb.is_secret_variable_name(tok.text)) {
                        fp.has_secret_access = true;
                        fp.accessed_variables.push_back(tok.text);
                    }
                }
            }
        ...
        fp.hash = compute_structural_hash(fp);
        fp.pattern_class = classify_pattern(fp);
        return fp;
    }

    bool is_structurally_novel(const CodeFingerprint& fp,
                                const std::vector<CodeFingerprint>& known_fps) const {
        for (const auto& known : known_fps) {
            if (fp.hash == known.hash) return false;
            if (fp.pattern_class == known.pattern_class &&
                fp.branch_count == known.branch_count &&
                fp.called_functions.size() == known.called_functions.size()) {
                int common_calls = 0;
                for (const auto& c : fp.called_functions) {
                    for (const auto& k : known.called_functions) {
                        if (c == k) { common_calls++; break; }
                    }
                }
                double similarity = static_cast<double>(common_calls) /
                                   std::max(fp.called_functions.size(), static_cast<size_t>(1));
...
private:
    uint64_t compute_structural_hash(const CodeFingerprint& fp) const {
        uint64_t h = 14695981039346656037ULL;
        auto hash_str = [&](const std::string& s) {
            for (char c : s) { h ^= static_cast<uint64_t>(c); h *= 1099511628211ULL; }
        };
        hash_str(fp.pattern_class);
        h ^= fp.branch_count * 31;
        h ^= fp.loop_count * 37;
        h ^= fp.has_error_handling ? 0x1234 : 0;
        h ^= fp.has_crypto_ops ? 0x5678 : 0;
        for (const auto& c : fp.called_functions) hash_str(c);
        return h;
    }

    std::string classify_pattern(const CodeFingerprint& fp) const {
        if (fp.has_crypto_ops && fp.has_secret_access) return "crypto_secret_handler";
        if (fp.has_crypto_ops) return "crypto_operation";
        if (fp.has_secret_access && fp.has_error_handling) return "secret_error_path";
        if (fp.has_secret_access) return "secret_accessor";
        if (fp.branch_count > 10) return "complex_validation";
        return "general";
    }
};

// ============================================================================
// SECTION 47: NOVELTY CONFIDENCE SCORER
// ============================================================================
...
public:
    struct NoveltyScore {
        ...
        bool matches_known_issue;
        std::string matched_cve;
        bool is_structurally_novel;
        std::string explanation;
    };

    NoveltyScore score(const Finding& finding) const {
        NoveltyScore ns;
        ns.raw_score = finding.confidence;
        ns.matches_known_issue = HistoricalIssueDatabase::instance().matches_known_issue(finding);
        ns.matched_cve = HistoricalIssueDatabase::instance().get_matching_cve(finding);
        ns.is_structurally_novel = !ns.matches_known_issue;

        if (ns.matches_known_issue) {
            ns.adjusted_score = ns.raw_score * 0.1;
            ns.novelty_status = "KNOWN_HISTORICAL";
            ns.explanation = "Matches known issue: " + ns.matched_cve;
        } else if (HistoricalIssueDatabase::instance().is_known_class(finding.evidence)) {
            ns.adjusted_score = ns.raw_score * 0.3;
            ns.novelty_status = "KNOWN_CLASS";
            ns.explanation = "Matches known vulnerability class pattern";
        } else {
            ns.adjusted_score = ns.raw_score;
            ns.novelty_status = "POTENTIALLY_NOVEL";
            ns.explanation = "Does not match any known historical issue or class";
            if (finding.severity == Severity::Critical) {
                ns.adjusted_score = std::min(1.0, ns.adjusted_score * 1.2);
                ns.novelty_status = "NOVEL_HIGH_PRIORITY";
            }
        }
        return ns;
    }
...
// ============================================================================
// SECTION 48: DUPLICATE ROOT CAUSE COLLAPSER
// ============================================================================
// Collapses many findings with the same root cause into a single finding
...
public:
    struct RootCause {
        ...
        std::string description;
        IssueType issue_type;
        std::vector<uint64_t> finding_ids;
        double aggregated_confidence;
        Severity max_severity;
        std::set<std::string> affected_files;
        std::set<std::string> affected_functions;
    };

    std::vector<Finding> collapse(const std::vector<Finding>& findings) {
        auto root_causes = identify_root_causes(findings);
        std::vector<Finding> collapsed;
        std::set<uint64_t> consumed;

        for (const auto& [cause_id, rc] : root_causes) {
            if (rc.finding_ids.size() <= 1) continue;
            Finding representative;
            bool found_rep = false;
            for (const auto& f : findings) {
                if (f.finding_id == rc.finding_ids[0]) {
                    representative = f;
                    found_rep = true;
                    break;
                }
            ...
            representative.confidence = std::min(1.0, rc.aggregated_confidence);
            representative.severity = rc.max_severity;
            representative.evidence += " [COLLAPSED: " + std::to_string(rc.finding_ids.size()) +
                                      " instances share root cause '" + rc.description + "']";
            std::string files_str;
            int count = 0;
            for (const auto& af : rc.affected_files) {
                if (count++ > 0) files_str += ", ";
                files_str += af.substr(af.rfind('/') + 1);
                if (count >= 5) { files_str += "..."; break; }
            }
            representative.evidence += " Affected files: " + files_str;
            collapsed.push_back(representative);
            for (uint64_t id : rc.finding_ids) consumed.insert(id);
        }

        for (const auto& f : findings) {
            if (consumed.find(f.finding_id) == consumed.end()) {
...
private:
    std::map<std::string, RootCause> identify_root_causes(const std::vector<Finding>& findings) {
        std::map<std::string, RootCause> causes;
        for (const auto& f : findings) {
            std::string cause_id = compute_cause_id(f);
            auto& rc = causes[cause_id];
            if (rc.finding_ids.empty()) {
                rc.cause_id = cause_id;
                rc.description = f.issue_type_string();
                rc.issue_type = f.issue_type;
                rc.aggregated_confidence = 0.0;
                rc.max_severity = f.severity;
            }
            rc.finding_ids.push_back(f.finding_id);
            rc.aggregated_confidence = std::max(rc.aggregated_confidence, f.confidence);
            if (f.severity < rc.max_severity) rc.max_severity = f.severity;
            rc.affected_files.insert(f.file);
            rc.affected_functions.insert(f.function_name);
        }
        return causes;
    }

    std::string compute_cause_id(const Finding& f) const {
        return f.issue_type_string() + "|" + f.secret_type_string() + "|" +
               (f.file.find("0.14") != std::string::npos ? "0.14" : "24.0");
    }
};

// ============================================================================
// SECTION 49: WALLET SECRET LIFETIME TRACKER
// ============================================================================
// Tracks the precise lifetime of every secret variable from creation to
// destruction across function boundaries, detecting lifetime extension bugs.
...
public:
    ...
    std::vector<Finding> analyze(const std::shared_ptr<TranslationUnit>& tu,
        ...
        auto lifetimes = track_all_secrets(tu);
        for (const auto& lt : lifetimes) {
            evaluate_lifetime(lt, release_name, findings);
        }
        return findings;
    }

private:
    std::vector<SecretLifetime> track_all_secrets(const std::shared_ptr<TranslationUnit>& tu) {
        std::vector<SecretLifetime> lifetimes;
        auto& kb = SecretTypeKnowledgeBase::instance();
        const auto& secret_types = kb.get_secret_types();
        ...
        std::string current_func;
        int brace_depth = 0;
        std::map<std::string, SecretLifetime> active_secrets;
        ...
        while (std::getline(stream, line)) {
            ...
            for (char c : line) {
                ...
                if (c == '}') {
                    brace_depth--;
                    if (brace_depth <= 0) {
                        for (auto& [name, lt] : active_secrets) {
                            lt.scope_end_line = line_num;
                            if (!lt.is_wiped) lifetimes.push_back(lt);
                        }
            ...
            if (line.find("(") != std::string::npos && line.find("{") != std::string::npos &&
                ...
                if (paren > 0) {
                    ...
                    if (name_start != std::string::npos) {
                        current_func = line.substr(name_start + 1, paren - name_start - 1);
                        size_t scope_pos = current_func.find("::");
                        if (scope_pos != std::string::npos) {
                            current_func = current_func.substr(scope_pos + 2);
                        }
                    }
                }
            }

            for (const auto& st : secret_types) {
                if (line.find(st) != std::string::npos) {
                    std::regex decl_re(st + R"(\s+(\w+))");
                    std::smatch match;
                    if (std::regex_search(line, match, decl_re)) {
                        std::string var_name = match[1].str();
                        if (var_name != "=" && var_name != "(" && var_name.size() > 1) {
                            SecretLifetime lt;
                            lt.variable_name = var_name;
                            lt.type_name = st;
                            lt.creating_function = current_func;
                            lt.creation_line = line_num;
                            lt.last_use_line = line_num;
                            lt.wipe_line = 0;
                            lt.scope_end_line = 0;
                            lt.is_wiped = false;
                            lt.escapes_scope = false;
                            lt.crosses_function_boundary = false;
                            lt.survives_exception = false;
                            lt.on_heap = (line.find("new ") != std::string::npos ||
                                         line.find("make_shared") != std::string::npos ||
                                         line.find("make_unique") != std::string::npos);
                            lt.in_container = (line.find("vector") != std::string::npos ||
                                             line.find("map") != std::string::npos);
                            lt.copy_count = 0;
                            lt.file_path = tu->file_path;
                            active_secrets[var_name] = lt;
                        }
                    }
                }
            }
            ...
            for (auto& [name, lt] : active_secrets) {
                if (line.find(name) != std::string::npos && line_num > lt.creation_line) {
                    lt.last_use_line = line_num;
                    if (line.find("memory_cleanse") != std::string::npos ||
                        line.find("OPENSSL_cleanse") != std::string::npos) {
                        lt.is_wiped = true;
                        lt.wipe_line = line_num;
                    }
                    if (line.find("return ") != std::string::npos && line.find(name) != std::string::npos) {
                        lt.escapes_scope = true;
                        lt.crosses_function_boundary = true;
                    }
                    if (line.find("=") != std::string::npos && line.find(name) != std::string::npos) {
                        size_t eq_pos = line.find("=");
                        size_t name_pos = line.find(name);
                        if (name_pos > eq_pos) lt.copy_count++;
                    }
                    if (line.find("push_back") != std::string::npos || line.find("emplace") != std::string::npos) {
                        lt.in_container = true;
                        lt.copy_count++;
                    }
                }
            }

            if (line.find("catch") != std::string::npos || line.find("throw") != std::string::npos) {
                for (auto& [name, lt] : active_secrets) {
                    if (!lt.is_wiped) lt.survives_exception = true;
                }
            }
        }
    ...
    void evaluate_lifetime(const SecretLifetime& lt, const std::string& release,
                           std::vector<Finding>& findings) {
        uint32_t lifetime_span = lt.last_use_line - lt.creation_line;

        if (!lt.is_wiped && lifetime_span > 3) {
            Finding f;
            f.finding_id = IDGenerator::instance().next();
            f.release = release;
            f.file = lt.file_path;
            f.function_name = lt.creating_function;
            f.secret_material_type = SecretTypeKnowledgeBase::instance().classify_secret(lt.type_name);
            f.location = SourceLocation(lt.file_path, lt.creation_line, 1);

            if (lt.escapes_scope) {
                f.issue_type = IssueType::DuplicateSecretCopy;
                f.severity = Severity::High;
                f.confidence = 0.75;
                f.evidence = "Secret '" + lt.variable_name + "' (type: " + lt.type_name +
                            ") escapes scope via return in " + lt.creating_function +
                            " without wipe. Creates untracked copy.";
            } else if (lt.survives_exception) {
                f.issue_type = IssueType::ExceptionPathRetention;
                f.severity = Severity::High;
                f.confidence = 0.80;
                f.evidence = "Secret '" + lt.variable_name + "' (type: " + lt.type_name +
                            ") survives exception path in " + lt.creating_function +
                            " without wipe at line " + std::to_string(lt.creation_line) + ".";
            } else if (lt.on_heap) {
                f.issue_type = IssueType::HeapRetainedPrivateKey;
                f.severity = Severity::High;
                f.confidence = 0.78;
                f.evidence = "Heap-allocated secret '" + lt.variable_name + "' (type: " + lt.type_name +
                            ") not wiped. Persists in heap after deallocation. Lifetime: " +
                            std::to_string(lifetime_span) + " lines.";
            } else if (lt.copy_count > 1) {
                f.issue_type = IssueType::DuplicateSecretCopy;
                f.severity = Severity::Medium;
                f.confidence = 0.65;
                f.evidence = "Secret '" + lt.variable_name + "' copied " +
                            std::to_string(lt.copy_count) + " times without wipe of originals.";
            } else {
                f.issue_type = IssueType::StaleDecryptedKey;
                f.severity = Severity::Medium;
                f.confidence = 0.60;
                f.evidence = "Secret '" + lt.variable_name + "' (type: " + lt.type_name +
                            ") not wiped. Lifetime: " + std::to_string(lifetime_span) + " lines in " +
                            lt.creating_function + ".";
            }
            f.classification = Classification::Inconclusive;
            f.manual_review_required = true;
...
// ============================================================================
// SECTION 50: ORACLE ATTACK DETECTOR (DEEP)
// ============================================================================
// Hunts specifically for padding oracle, timing oracle, and error oracle
// vulnerabilities in wallet encryption/decryption paths.

class OracleAttackDetector {
public:
    ...
    std::vector<Finding> analyze(const std::shared_ptr<TranslationUnit>& tu,
        ...
        detect_padding_oracle_pattern(tu, release_name, findings);
        detect_timing_oracle(tu, release_name, findings);
        detect_error_code_oracle(tu, release_name, findings);
        detect_branch_oracle(tu, release_name, findings);
        detect_allocation_size_oracle(tu, release_name, findings);
        detect_exception_oracle(tu, release_name, findings);
        return findings;
    }

private:
    void detect_padding_oracle_pattern(const std::shared_ptr<TranslationUnit>& tu,
                                        const std::string& release,
                                        std::vector<Finding>& findings) {
        if (tu->file_path.find("crypter") == std::string::npos &&
            tu->file_path.find("wallet") == std::string::npos) return;

        std::istringstream stream(tu->raw_content);
        std::string line;
        uint32_t line_num = 0;
        bool in_decrypt = false;
        bool found_final_call = false;
        bool found_bool_return = false;
        bool found_length_check = false;

        while (std::getline(stream, line)) {
            line_num++;
            if (line.find("Decrypt") != std::string::npos &&
                (line.find("bool") != std::string::npos || line.find("{") != std::string::npos)) {
                in_decrypt = true;
                found_final_call = false;
                found_bool_return = false;
                found_length_check = false;
            }
            if (in_decrypt) {
                if (line.find("EVP_DecryptFinal") != std::string::npos ||
                    line.find("DecryptFinal") != std::string::npos) {
                    found_final_call = true;
                }
                if (line.find("return false") != std::string::npos ||
                    line.find("return true") != std::string::npos) {
                    found_bool_return = true;
                }
                if (line.find("nLen") != std::string::npos || line.find("outl") != std::string::npos ||
                    line.find("out_len") != std::string::npos) {
                    found_length_check = true;
                }
                if (line.find("}") != std::string::npos && found_final_call && found_bool_return) {
                    in_decrypt = false;
                    Finding f;
                    f.finding_id = IDGenerator::instance().next();
                    f.release = release;
                    f.file = tu->file_path;
                    f.function_name = "Decrypt";
                    f.issue_type = IssueType::AllocatorReuseLeakage;
                    f.classification = Classification::Inconclusive;
                    f.secret_material_type = SecretMaterialType::WalletPassword;
                    f.severity = Severity::Critical;
                    f.reachability = "rpc_walletpassphrase";
                    f.confidence = 0.75;
                    f.location = SourceLocation(tu->file_path, line_num, 1);
                    f.evidence = "PADDING ORACLE CANDIDATE: Decrypt function calls EVP_DecryptFinal "
                                "(which checks PKCS#7 padding) and returns bool. The true/false "
                                "return value distinguishes valid-padding from invalid-padding. "
                                "If this distinction is observable externally (via RPC error codes, "
                                "timing, or exception behavior), a padding oracle attack decrypts "
                                "the master key in ~384 queries. "
                                "VERDICT: Requires runtime testing with poc_rpc_oracle_test.py "
                                "to determine if the oracle is externally observable. " +
                                std::string(found_length_check ?
                                "Output length differences may also serve as oracle." :
                                "No output length oracle detected.");
                    f.manual_review_required = true;
                    findings.push_back(f);
                }
            }
    ...
    void detect_timing_oracle(const std::shared_ptr<TranslationUnit>& tu,
                               const std::string& release,
                               std::vector<Finding>& findings) {
        if (tu->file_path.find("crypter") == std::string::npos &&
            tu->file_path.find("wallet") == std::string::npos &&
            tu->file_path.find("http") == std::string::npos) return;
        ...
        while (std::getline(stream, line)) {
            line_num++;
            if ((line.find("Unlock") != std::string::npos || line.find("Decrypt") != std::string::npos) &&
                line.find("for") != std::string::npos && line.find("mapCryptedKeys") != std::string::npos) {
                Finding f;
                f.finding_id = IDGenerator::instance().next();
                f.release = release;
                f.file = tu->file_path;
                ...
                f.secret_material_type = SecretMaterialType::WalletPassword;
                f.severity = Severity::High;
                f.reachability = "timing_channel";
                f.confidence = 0.60;
                f.location = SourceLocation(tu->file_path, line_num, 1);
                f.evidence = "TIMING ORACLE CANDIDATE: Unlock iterates over mapCryptedKeys "
                            "and decrypts each key. The time to complete depends on the number "
                            "of keys in the wallet. A correct passphrase processes all keys; "
                            "an incorrect passphrase may fail on the first key. This timing "
                            "difference reveals whether decryption is proceeding (correct pass) "
                            "or failing immediately (wrong pass). Combined with PBKDF2 brute-force, "
                            "this could speed up offline cracking by confirming correct derivation.";
                f.manual_review_required = true;
                findings.push_back(f);
            }
        }
    ...
    void detect_error_code_oracle(const std::shared_ptr<TranslationUnit>& tu,
                                   const std::string& release,
                                   std::vector<Finding>& findings) {
        bool is_rpc = tu->file_path.find("rpcwallet") != std::string::npos ||
                     tu->file_path.find("wallet/rpc") != std::string::npos;
        if (!is_rpc) return;

        std::istringstream stream(tu->raw_content);
        std::string line;
        uint32_t line_num = 0;
        bool in_passphrase_handler = false;
        std::set<std::string> error_codes;
        ...
        while (std::getline(stream, line)) {
            ...
            if (line.find("walletpassphrase") != std::string::npos &&
                (line.find("\"walletpassphrase\"") != std::string::npos ||
                 line.find("walletpassphrase(") != std::string::npos)) {
                in_passphrase_handler = true;
                handler_start = line_num;
                error_codes.clear();
            }
            if (in_passphrase_handler) {
                std::regex rpc_err(R"(RPC_\w+)");
                auto begin = std::sregex_iterator(line.begin(), line.end(), rpc_err);
                auto end = std::sregex_iterator();
                for (auto it = begin; it != end; ++it) {
                    error_codes.insert((*it).str());
                }
                if (line_num > handler_start + 80) {
                    in_passphrase_handler = false;
                    if (error_codes.size() >= 2) {
                        Finding f;
                        f.finding_id = IDGenerator::instance().next();
                        f.release = release;
                        f.file = tu->file_path;
                        f.function_name = "walletpassphrase";
                        f.issue_type = IssueType::RPCPasswordExposure;
                        f.classification = Classification::Inconclusive;
                        f.secret_material_type = SecretMaterialType::WalletPassword;
                        f.severity = Severity::Critical;
                        f.reachability = "rpc_error_code_oracle";
                        f.confidence = 0.80;
                        f.location = SourceLocation(tu->file_path, handler_start, 1);
                        std::string codes;
                        for (const auto& c : error_codes) {
                            if (!codes.empty()) codes += ", ";
                            codes += c;
                        }
                        f.evidence = "ERROR CODE ORACLE: walletpassphrase handler returns " +
                                    std::to_string(error_codes.size()) + " distinct RPC error codes: " +
                                    codes + ". If these distinguish between 'wrong passphrase' (successful "
                                    "AES decryption but key mismatch) and 'corrupt data' (padding error), "
                                    "this is the padding oracle. Test with poc_rpc_oracle_test.py.";
                        f.manual_review_required = true;
                        findings.push_back(f);
                    }
                }
    ...
    void detect_branch_oracle(const std::shared_ptr<TranslationUnit>& tu,
                               const std::string& release,
                               std::vector<Finding>& findings) {
        if (tu->file_path.find("crypter") == std::string::npos) return;
        std::istringstream stream(tu->raw_content);
        ...
        while (std::getline(stream, line)) {
            line_num++;
            if (line.find("EVP_DecryptUpdate") != std::string::npos) {
                bool has_branch_after = false;
                std::istringstream fwd(tu->raw_content);
                std::string fline; uint32_t fln = 0;
                while (std::getline(fwd, fline)) { fln++;
                    if (fln > line_num && fln <= line_num + 5) {
                        if (fline.find("if") != std::string::npos && fline.find("!") != std::string::npos) {
                            has_branch_after = true;
                        }
                    }
                }
                if (has_branch_after) {
                    ...
                    f.secret_material_type = SecretMaterialType::EncryptionKey;
                    f.severity = Severity::High;
                    f.reachability = "branch_side_channel";
                    f.confidence = 0.55;
                    f.location = SourceLocation(tu->file_path, line_num, 1);
                    f.evidence = "BRANCH ORACLE: EVP_DecryptUpdate followed by conditional branch. "
                                "Branch taken/not-taken after partial decryption may leak information "
                                "about plaintext via cache timing side-channel.";
                    f.manual_review_required = true;
                    findings.push_back(f);
                }
            }
    ...
    void detect_allocation_size_oracle(const std::shared_ptr<TranslationUnit>& tu,
                                        const std::string& release,
                                        std::vector<Finding>& findings) {
        if (tu->file_path.find("crypter") == std::string::npos) return;
        std::istringstream stream(tu->raw_content);
        ...
        while (std::getline(stream, line)) {
            ...
            if (line.find("resize") != std::string::npos &&
                (line.find("Decrypt") != std::string::npos || line.find("plaintext") != std::string::npos ||
                 line.find("vchPlaintext") != std::string::npos)) {
                Finding f;
                f.finding_id = IDGenerator::instance().next();
                f.release = release;
                f.file = tu->file_path;
                ...
                f.secret_material_type = SecretMaterialType::DecryptedSecret;
                f.severity = Severity::Medium;
                f.reachability = "allocation_side_channel";
                f.confidence = 0.45;
                f.location = SourceLocation(tu->file_path, line_num, 1);
                f.evidence = "ALLOCATION SIZE ORACLE: Output buffer resized based on decryption result. "
                            "If the resize amount differs between valid and invalid padding, an observer "
                            "monitoring heap allocation patterns can distinguish padding validity.";
                f.manual_review_required = true;
                findings.push_back(f);
            }
        }
    ...
    void detect_exception_oracle(const std::shared_ptr<TranslationUnit>& tu,
                                  const std::string& release,
                                  std::vector<Finding>& findings) {
        if (tu->file_path.find("crypter") == std::string::npos &&
            tu->file_path.find("rpcwallet") == std::string::npos) return;
        bool has_try_around_decrypt = false;
        std::istringstream stream(tu->raw_content);
        std::string line;
        uint32_t line_num = 0;
        bool in_try = false;
        while (std::getline(stream, line)) {
            line_num++;
            if (line.find("try") != std::string::npos && line.find("{") != std::string::npos) in_try = true;
            if (in_try && (line.find("Decrypt") != std::string::npos || line.find("Unlock") != std::string::npos)) {
                has_try_around_decrypt = true;
            }
            if (line.find("catch") != std::string::npos && has_try_around_decrypt) {
                Finding f;
                f.finding_id = IDGenerator::instance().next();
                f.release = release;
                f.file = tu->file_path;
                ...
                f.secret_material_type = SecretMaterialType::WalletPassword;
                f.severity = Severity::High;
                f.reachability = "exception_oracle";
                f.confidence = 0.55;
                f.location = SourceLocation(tu->file_path, line_num, 1);
                f.evidence = "EXCEPTION ORACLE: Decrypt/Unlock wrapped in try-catch. If decryption "
                            "throws different exceptions for padding errors vs key errors, the catch "
                            "handler behavior serves as an oracle distinguisher.";
                f.manual_review_required = true;
                findings.push_back(f);
                has_try_around_decrypt = false;
                in_try = false;
            }
        }
    }
...
// ============================================================================
// SECTION 51: EXPLOIT TRACE RECONSTRUCTOR
// ============================================================================

class ExploitTraceReconstructor {
public:
    struct ExploitTrace {
    ...
    ExploitTrace reconstruct(const Finding& finding) {
        ExploitTrace trace;
        trace.feasibility = finding.confidence;

        switch (finding.issue_type) {
            case IssueType::CrashDumpPersistence:
                if (finding.evidence.find("BDB") != std::string::npos) {
                    trace.exploit_name = "BDB_DELETED_KEY_RECOVERY";
                    trace.precondition = "Attacker has copy of wallet.dat (any version after encryption)";
                    trace.steps = {
                        "1. Obtain wallet.dat file (backup, theft, forensic recovery)",
                        "2. Parse BDB btree pages at page_size intervals (4096 or 16384 bytes)",
                        "3. Scan each page for \\x03key marker followed by 33/65-byte compressed/uncompressed pubkey",
                        "4. After pubkey, extract DER-encoded private key (look for \\x04\\x20 + 32 bytes)",
                        "5. Verify extracted key by deriving public key and comparing",
                        "6. Import recovered key to new wallet: bitcoin-cli importprivkey <WIF>"
                    };
                    trace.postcondition = "All private keys from before encryption are recovered";
                    trace.tool_command = "python3 poc_bdb_key_recovery.py wallet.dat";
                } else {
                    trace.exploit_name = "MEMORY_DUMP_RECOVERY";
                    trace.precondition = "Core dump or memory dump exists from crash while wallet unlocked";
                    trace.steps = {"1. Obtain core dump", "2. Scan for 32-byte AES key pattern", "3. Decrypt ckey records"};
                    trace.postcondition = "Master key recovered from crash dump";
                    trace.tool_command = "strings core.PID | grep -P '[\\x00-\\xff]{32}'";
                }
                break;

            case IssueType::AllocatorReuseLeakage:
                if (finding.evidence.find("PBKDF2") != std::string::npos) {
                    trace.exploit_name = "PBKDF2_OFFLINE_BRUTEFORCE";
                    trace.precondition = "Attacker has wallet.dat file";
                    trace.steps = {
                        "1. Extract mkey record from wallet.dat",
                        "2. Parse: encrypted_master_key(48B) + salt(8B) + method(4B) + iterations(4B)",
                        "3. Convert to hashcat format: $bitcoin$48$<hex_key>$8$<hex_salt>$<iterations>",
                        "4. Run: hashcat -m 11300 hash.txt wordlist.txt -r rules/best64.rule",
                        "5. Recovered passphrase decrypts master key",
                        "6. Master key decrypts all ckey records -> all private keys"
                    };
                    trace.postcondition = "All wallet private keys recovered";
                    trace.tool_command = "python3 poc_mkey_extract.py wallet.dat && hashcat -m 11300 wallet_hash.txt";
                } else if (finding.evidence.find("oracle") != std::string::npos ||
                           finding.evidence.find("Oracle") != std::string::npos ||
                           finding.evidence.find("ORACLE") != std::string::npos) {
                    trace.exploit_name = "PADDING_ORACLE_ATTACK";
                    trace.precondition = "Attacker has RPC access + wallet.dat copy";
                    trace.steps = {
                        "1. Extract encrypted master key from wallet.dat mkey record",
                        "2. For each byte position (48 bytes, 3 AES blocks):",
                        "   a. Modify ciphertext byte",
                        "   b. Submit to bitcoind via walletpassphrase with dummy password",
                        "   c. Observe response: error -14 (valid padding) vs other (invalid padding)",
                        "   d. Use padding validity to derive intermediate state",
                        "3. XOR intermediate state with known ciphertext to recover plaintext",
                        "4. After ~384 queries: complete master key recovered",
                        "5. Decrypt all private keys offline"
                    };
                    trace.postcondition = "Master key + all private keys without knowing passphrase";
                    trace.tool_command = "python3 poc_rpc_oracle_test.py wallet.dat localhost rpcuser rpcpass";
                } else {
                    trace.exploit_name = "SECRET_MEMORY_PERSISTENCE";
                    trace.precondition = "Local access to bitcoind process";
                    trace.steps = {"1. Attach debugger to bitcoind", "2. Scan heap for secret patterns"};
                    trace.postcondition = "Secret material recovered from process memory";
                    trace.tool_command = "gdb -p $(pidof bitcoind) -ex 'dump memory heap.bin 0x START 0x END'";
                }
                break;

            case IssueType::IntegerOverflow:
                if (finding.evidence.find("CVE-2018-17144") != std::string::npos ||
                    finding.evidence.find("duplicate input") != std::string::npos) {
                    trace.exploit_name = "CVE_2018_17144_INFLATION";
                    trace.precondition = "Mining capability or direct block submission";
                    trace.steps = {
                        "1. Create transaction with vin[0].prevout == vin[1].prevout",
                        "2. Sign transaction normally (only vin[0] needs valid signature)",
                        "3. Include in mined block or submit via submitblock",
                        "4. Input value counted twice -> coins created from nothing"
                    };
                    trace.postcondition = "Arbitrary inflation of money supply";
                    trace.tool_command = "python3 poc_cve_2018_17144.py bitcoin-cli";
                } else {
                    trace.exploit_name = "VALUE_OVERFLOW";
                    trace.precondition = "Ability to create crafted transactions";
                    trace.steps = {"1. Craft transaction with values summing near int64 max", "2. Submit to mempool"};
                    trace.postcondition = "Potential inflation via integer wraparound";
                    trace.tool_command = "Manual crafting required";
                }
                break;

            default:
                trace.exploit_name = "GENERIC_" + finding.issue_type_string();
                trace.steps = {"Manual analysis required"};
                trace.tool_command = "Manual";
                break;
        }
        return trace;
    }
...
// ============================================================================
// SECTION 52: NOVELTY ANALYSIS ENGINE (MASTER COORDINATOR)
// ============================================================================
// The top-level novelty engine that orchestrates all sub-modules to find
// genuinely novel vulnerabilities while suppressing known historical issues.

class NoveltyAnalysisEngine {
public:
    struct NoveltyResult {
        Finding finding;
        NoveltyConfidenceScorer::NoveltyScore novelty;
        ExploitTraceReconstructor::ExploitTrace trace;
        bool is_novel;
        bool should_emit;
    };

    std::vector<NoveltyResult> analyze(const std::vector<Finding>& raw_findings) {
        std::vector<NoveltyResult> results;
        NoveltyConfidenceScorer scorer;
        ExploitTraceReconstructor trace_gen;
        DuplicateRootCauseCollapser collapser;

        auto collapsed = collapser.collapse(raw_findings);
        Logger::instance().info("Root cause collapse: " + std::to_string(raw_findings.size()) +
                               " -> " + std::to_string(collapsed.size()) + " distinct findings");

        for (const auto& f : collapsed) {
            NoveltyResult nr;
            nr.finding = f;
            nr.novelty = scorer.score(f);
            nr.trace = trace_gen.reconstruct(f);
            nr.is_novel = (nr.novelty.novelty_status == "POTENTIALLY_NOVEL" ||
                          nr.novelty.novelty_status == "NOVEL_HIGH_PRIORITY");
            nr.should_emit = (nr.novelty.adjusted_score >= 0.15);

            if (nr.is_novel && f.severity == Severity::Critical) {
                nr.finding.confidence = nr.novelty.adjusted_score;
                nr.finding.evidence = "[" + nr.novelty.novelty_status + "] " + nr.finding.evidence;
            } else if (nr.novelty.matches_known_issue) {
                nr.finding.confidence = nr.novelty.adjusted_score;
                nr.finding.evidence = "[KNOWN: " + nr.novelty.matched_cve + "] " + nr.finding.evidence;
                nr.finding.classification = Classification::Inconclusive;
            }
            results.push_back(nr);
        }

        int novel_count = 0, known_count = 0, emitted = 0;
        for (const auto& nr : results) {
            if (nr.is_novel) novel_count++;
            else known_count++;
            if (nr.should_emit) emitted++;
        }
        Logger::instance().info("Novelty analysis: " + std::to_string(novel_count) + " novel, " +
                               std::to_string(known_count) + " known, " +
                               std::to_string(emitted) + " emitted");
        return results;
    }

    std::vector<Finding> get_emittable_findings(const std::vector<NoveltyResult>& results) {
        std::vector<Finding> emittable;
        for (const auto& nr : results) {
            if (nr.should_emit) {
                emittable.push_back(nr.finding);
            }
        }
        std::sort(emittable.begin(), emittable.end(), [](const Finding& a, const Finding& b) {
...
// ============================================================================
// SECTION 53: MODERN REGRESSION DETECTOR (0.14.1 vs 24.0.1)
// ============================================================================
// Finds vulnerabilities that were INTRODUCED after 0.14.1 by comparing
// code patterns between versions.

class ModernRegressionDetector {
public:
    std::vector<Finding> analyze(const std::map<std::string, ReleaseInfo>& releases) {
        std::vector<Finding> findings;
        const ReleaseInfo* old_rel = nullptr;
        const ReleaseInfo* new_rel = nullptr;
        for (const auto& [name, rel] : releases) {
            if (name.find("0.14") != std::string::npos) old_rel = &rel;
            if (name.find("24.") != std::string::npos) new_rel = &rel;
        }
        if (!old_rel || !new_rel) return findings;

        detect_new_attack_surface(old_rel, new_rel, findings);
        detect_weakened_validation(old_rel, new_rel, findings);
        detect_changed_crypto(old_rel, new_rel, findings);
        detect_sqlite_transition_risks(new_rel, findings);
        detect_descriptor_wallet_risks(new_rel, findings);
        return findings;
    }

private:
    void detect_new_attack_surface(const ReleaseInfo* old_rel, const ReleaseInfo* new_rel,
                                    std::vector<Finding>& findings) {
        std::set<std::string> old_files, new_files;
        for (const auto& f : old_rel->all_files) {
            old_files.insert(f.substr(f.rfind('/') + 1));
        }
        for (const auto& f : new_rel->all_files) {
            new_files.insert(f.substr(f.rfind('/') + 1));
        }
        std::vector<std::string> new_wallet_files;
        for (const auto& f : new_files) {
            if (old_files.find(f) == old_files.end()) {
                if (f.find("wallet") != std::string::npos || f.find("script") != std::string::npos ||
                    f.find("descriptor") != std::string::npos || f.find("spend") != std::string::npos) {
                    new_wallet_files.push_back(f);
                }
            }
        }
        if (!new_wallet_files.empty()) {
            Finding f;
            f.finding_id = IDGenerator::instance().next();
            f.release = new_rel->name;
            f.issue_type = IssueType::UnboundedKeyLifetime;
            f.classification = Classification::Inconclusive;
            f.secret_material_type = SecretMaterialType::PrivateKey;
            f.severity = Severity::Medium;
            f.confidence = 0.55;
            std::string files_str;
            for (size_t i = 0; i < std::min(new_wallet_files.size(), static_cast<size_t>(10)); i++) {
                if (i > 0) files_str += ", ";
                files_str += new_wallet_files[i];
            }
            f.evidence = "NEW ATTACK SURFACE: " + std::to_string(new_wallet_files.size()) +
                        " wallet/script files added in 24.0.1 that don't exist in 0.14.1: " +
                        files_str + ". Each new file may introduce secret handling that hasn't "
                        "received the same scrutiny as the original wallet code.";
            f.manual_review_required = true;
            findings.push_back(f);
        }
    }

    void detect_weakened_validation(const ReleaseInfo* old_rel, const ReleaseInfo* new_rel,
                                     std::vector<Finding>& findings) {
        auto count_pattern = [](const ReleaseInfo* rel, const std::string& pattern) -> int {
            int count = 0;
            for (const auto& [path, tu] : rel->translation_units) {
                if (!tu) continue;
                size_t pos = 0;
                while ((pos = tu->raw_content.find(pattern, pos)) != std::string::npos) {
                    count++; pos += pattern.size();
                }
            }
            return count;
        };
        ...
        std::vector<PatternCheck> checks = {
            {"assert(", "Assertions may be compiled out in release builds"},
            {"MoneyRange", "Money range validation"},
            {"CheckTransaction", "Transaction validation calls"},
            {"VerifyScript", "Script verification calls"},
            {"CheckInputs", "Input validation calls"},
        };

        for (const auto& check : checks) {
            int old_count = count_pattern(old_rel, check.pattern);
            int new_count = count_pattern(new_rel, check.pattern);
            double old_per_kloc = (old_rel->total_lines > 0) ?
                                 (1000.0 * old_count / old_rel->total_lines) : 0;
            double new_per_kloc = (new_rel->total_lines > 0) ?
                                 (1000.0 * new_count / new_rel->total_lines) : 0;
            if (old_per_kloc > 0 && new_per_kloc < old_per_kloc * 0.5) {
                Finding f;
                f.finding_id = IDGenerator::instance().next();
                f.release = new_rel->name;
                f.issue_type = IssueType::ConditionalWipeBypass;
                ...
                f.severity = Severity::Medium;
                f.confidence = 0.50;
                f.evidence = "WEAKENED VALIDATION: '" + check.pattern + "' density decreased from " +
                            std::to_string(old_per_kloc).substr(0, 4) + "/KLOC to " +
                            std::to_string(new_per_kloc).substr(0, 4) + "/KLOC between versions. " +
                            check.risk;
                f.manual_review_required = true;
                findings.push_back(f);
            }
    ...
    void detect_changed_crypto(const ReleaseInfo* old_rel, const ReleaseInfo* new_rel,
                                std::vector<Finding>& findings) {
        for (const auto& [path, tu] : new_rel->translation_units) {
            if (!tu || tu->file_path.find("crypter") == std::string::npos) continue;
            bool still_uses_sha1 = tu->raw_content.find("HMAC_SHA1") != std::string::npos ||
                                  tu->raw_content.find("PKCS5_PBKDF2_HMAC_SHA1") != std::string::npos;
            bool still_uses_cbc = tu->raw_content.find("aes_256_cbc") != std::string::npos ||
                                 tu->raw_content.find("EVP_aes_256_cbc") != std::string::npos;
            if (still_uses_sha1 && still_uses_cbc) {
                Finding f;
                f.finding_id = IDGenerator::instance().next();
                f.release = new_rel->name;
                f.file = tu->file_path;
                f.function_name = "CCrypter";
                f.issue_type = IssueType::AllocatorReuseLeakage;
                f.classification = Classification::Inconclusive;
                f.secret_material_type = SecretMaterialType::EncryptionKey;
                f.severity = Severity::High;
                f.confidence = 0.70;
                f.evidence = "CRYPTO NOT UPGRADED: Bitcoin Core 24.0.1 STILL uses PBKDF2-HMAC-SHA1 + "
                            "AES-256-CBC (same as 0.14.1). After 7+ years and multiple major versions, "
                            "no upgrade to Argon2/scrypt + AES-GCM/ChaCha20-Poly1305. The same offline "
                            "brute-force and padding oracle attack surfaces remain open.";
                f.manual_review_required = true;
                findings.push_back(f);
            }
        }
    ...
    void detect_sqlite_transition_risks(const ReleaseInfo* new_rel,
                                         std::vector<Finding>& findings) {
        for (const auto& [path, tu] : new_rel->translation_units) {
            if (!tu) continue;
            if (tu->file_path.find("sqlite") != std::string::npos &&
                tu->file_path.find("wallet") != std::string::npos) {
                if (tu->raw_content.find("INSERT") != std::string::npos &&
                    tu->raw_content.find("key") != std::string::npos) {
                    bool has_secure_delete = tu->raw_content.find("secure_delete") != std::string::npos ||
                                           tu->raw_content.find("PRAGMA secure_delete") != std::string::npos;
                    if (!has_secure_delete) {
                        Finding f;
                        f.finding_id = IDGenerator::instance().next();
                        f.release = new_rel->name;
                        f.file = tu->file_path;
                        ...
                        f.severity = Severity::High;
                        f.confidence = 0.65;
                        f.evidence = "SQLITE TRANSITION RISK: SQLite wallet handler writes key data "
                                    "without PRAGMA secure_delete. SQLite, like BDB, may retain deleted "
                                    "records on database pages. This is the BDB deleted-record bug "
                                    "replicated in the new storage backend.";
                        f.manual_review_required = true;
                        findings.push_back(f);
                    }
                }
    ...
    void detect_descriptor_wallet_risks(const ReleaseInfo* new_rel,
                                         std::vector<Finding>& findings) {
        for (const auto& [path, tu] : new_rel->translation_units) {
            if (!tu) continue;
            if (tu->file_path.find("scriptpubkeyman") != std::string::npos ||
                tu->file_path.find("descriptor") != std::string::npos) {
                if (tu->raw_content.find("GetKey") != std::string::npos ||
                    tu->raw_content.find("privkey") != std::string::npos) {
                    bool has_wipe = tu->raw_content.find("memory_cleanse") != std::string::npos ||
                                  tu->raw_content.find("OPENSSL_cleanse") != std::string::npos;
                    if (!has_wipe) {
                        Finding f;
                        f.finding_id = IDGenerator::instance().next();
                        f.release = new_rel->name;
                        ...
                        f.severity = Severity::High;
                        f.confidence = 0.60;
                        f.evidence = "DESCRIPTOR WALLET REGRESSION: New descriptor wallet key manager "
                                    "handles private keys without visible memory_cleanse calls. This is "
                                    "new code added after 0.14.1 that may lack the hardening present "
                                    "in the legacy key store.";
                        f.manual_review_required = true;
                        findings.push_back(f);
                    }
                }
...
// SECTION 45: HISTORICAL ISSUE DATABASE
// ============================================================================
// Suppresses known historical bugs so only NOVEL findings are emitted.

// ============================================================================
// SECTION 46: STRUCTURAL FINGERPRINT ENGINE
// ============================================================================
// Compares finding patterns against known bug structures to detect novelty.
...
// SECTION 48: DUPLICATE ROOT CAUSE COLLAPSER
// ============================================================================
// Collapses multiple findings that stem from the same root cause into one.

// ============================================================================
// SECTION 49: WALLET SECRET LIFETIME TRACKER
// ============================================================================
// Tracks the lifecycle of secret material through code paths with precision.
...
public:
    ...
    int run() {
        ...
        // Stage 20: Wallet Secret Lifetime Tracking
        Logger::instance().info("\n=== STAGE 20: WALLET SECRET LIFETIME TRACKING ===");
        Timer lifetime_timer;
        {
            WalletSecretLifetimeTracker lifetime_tracker;
            for (auto& [name, release] : releases_) {
                for (auto& [path, tu] : release.translation_units) {
                    if (!tu || tu->raw_content.empty()) continue;
                    auto lt_findings = lifetime_tracker.analyze(tu, name);
                    all_findings.insert(all_findings.end(), lt_findings.begin(), lt_findings.end());
                }
            }
        }
        Logger::instance().info("Lifetime tracking completed in " + lifetime_timer.elapsed_string() +
                               " (total: " + std::to_string(all_findings.size()) + ")");

        // Stage 21: Deep Oracle Attack Detection
        Logger::instance().info("\n=== STAGE 21: DEEP ORACLE ATTACK DETECTION ===");
        Timer deep_oracle_timer;
        {
            OracleAttackDetector oracle_detector;
            for (auto& [name, release] : releases_) {
                for (auto& [path, tu] : release.translation_units) {
                    if (!tu || tu->raw_content.empty()) continue;
                    auto oracle_findings = oracle_detector.analyze(tu, name);
                    all_findings.insert(all_findings.end(), oracle_findings.begin(), oracle_findings.end());
                }
            }
        }
        Logger::instance().info("Deep oracle detection completed in " + deep_oracle_timer.elapsed_string() +
                               " (total: " + std::to_string(all_findings.size()) + ")");

        // Stage 22: Modern Regression Detection
        Logger::instance().info("\n=== STAGE 22: MODERN REGRESSION DETECTION ===");
        Timer mod_reg_timer;
        {
            ModernRegressionDetector regression_detector;
            auto reg_findings = regression_detector.analyze(releases_);
            all_findings.insert(all_findings.end(), reg_findings.begin(), reg_findings.end());
        }
        Logger::instance().info("Regression detection completed in " + mod_reg_timer.elapsed_string() +
                               " (total: " + std::to_string(all_findings.size()) + ")");

        // Stage 23: Dynamic Instrumentation Generation
        Logger::instance().info("\n=== STAGE 23: DYNAMIC INSTRUMENTATION GENERATION ===");
        Timer dyn_timer;
        ...
        // Stage 26: Novelty Analysis - Suppress known issues, collapse duplicates
        Logger::instance().info("\n=== STAGE 26: NOVELTY ANALYSIS AND ROOT CAUSE COLLAPSE ===");
        Timer novelty_timer;
        NoveltyAnalysisEngine novelty_engine;
        auto novelty_report = novelty_engine.analyze(all_findings);
        all_findings = novelty_engine.get_emittable_findings(novelty_report);
        Logger::instance().info("Novelty analysis completed in " + novelty_timer.elapsed_string() +
                               " (emitting " + std::to_string(all_findings.size()) + " novel findings)");

        // Report Generation
        Logger::instance().info("\n=== STAGE 27: REPORT GENERATION ===");
...
public:
    static AnalysisConfig parse(int argc, char* argv[]) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (arg == "--verbose" || arg == "-v") { config.verbose = true; continue; }
            if (arg == "--output" || arg == "-o") { if (i+1<argc) config.output_path = argv[++i]; continue; }
            if (arg == "--threads" || arg == "-t") { if (i+1<argc) config.thread_count = std::stoi(argv[++i]); continue; }
            if (arg == "--no-fuzz") { config.enable_fuzz_generation = false; continue; }
            if (arg == "--no-diff") { config.enable_diff_analysis = false; continue; }
            if (arg == "--no-checkpoint") { config.enable_checkpoint = false; continue; }
            if (arg == "--checkpoint" || arg == "-c") { if (i+1<argc) config.checkpoint_path = argv[++i]; continue; }
            if (arg == "--min-confidence") { if (i+1<argc) config.min_confidence = std::stod(argv[++i]); continue; }
            if (arg == "--poc-test") { config.enable_poc_testing = true; continue; }
            if (arg == "--wallet-dat") { if (i+1<argc) config.wallet_dat_path = argv[++i]; continue; }
            if (arg == "--bitcoind") { if (i+1<argc) config.bitcoind_path = argv[++i]; continue; }
            if (arg == "--bitcoin-cli") { if (i+1<argc) config.bitcoin_cli_path = argv[++i]; continue; }
            if (arg == "--test") { run_self_tests(); std::exit(0); }
            if (arg == "-r" || arg == "--release") {
                if (i+1<argc) { std::string s=argv[++i]; size_t c=s.find(':');
                    if(c!=std::string::npos){config.release_names.push_back(s.substr(0,c));config.release_paths.push_back(s.substr(c+1));}
                    else{config.release_paths.push_back(s);config.release_names.push_back(s);} }
                continue;
            }
        ...
        if (config.output_path.empty()) config.output_path = "btc_audit_report.json";
        if (config.checkpoint_path.empty()) config.checkpoint_path = ".btc_audit_checkpoint";
        return config;
    }
    static void print_usage() {
        std::cout << "Bitcoin Core Adversarial Audit Framework\n\n"
                  << "Usage: btc_audit [options] release_path [release_path2 ...]\n\n"
                  << "Options:\n"
                  << "  -v, --verbose           Verbose output\n"
                  << "  -o, --output PATH       Output JSON report path\n"
                  << "  -t, --threads N         Number of threads\n"
                  << "  --poc-test              Run PoC verification against findings\n"
                  << "  --wallet-dat PATH       wallet.dat for PoC testing\n"
                  << "  --bitcoind PATH         bitcoind binary for PoC testing\n"
                  << "  --bitcoin-cli PATH      bitcoin-cli binary for PoC testing\n"
                  << "  --no-fuzz               Disable fuzz harness generation\n"
                  << "  --no-diff               Disable differential analysis\n"
                  << "  --test                  Run self-tests\n";
    }
